...
# Project's own CLI (so101_cli package), invoked as `dume run <subcommand>` (TUI is `dume`):
dume run find-ports                                            # identify which USB port is which arm (wraps lerobot-find-port)
dume run setup-motors follower|leader                          # assign motor IDs 1..6 (all, interactive)
dume run setup-motors follower|leader --motor gripper          # reconfigure ONE motor by joint name (recovery)
dume run scan-bus follower|leader                              # ping the bus; report which motor IDs 1..6 respond
dume run check [--no-leader] [--no-front] [--no-lateral]       # connectivity health check: both arms + both cameras
dume run follower move --pose home                              # move follower to a named pose
dume run follower move 0 -30 -60 0 0 0 --hold-time 2            # move to explicit joint angles (deg)
dume run follower move --tune                                   # interactive joint tuning
dume run follower replay paths/x.json                           # replay waypoints OR trajectory file
dume run leader record-waypoint paths/wp.json [--append]        # capture static poses ('s' to save, 'q' to finish)
dume run leader record-trajectory paths/t.json --rate 30        # capture continuous motion ('r' start/stop)
dume run teleop [--rate 30] [--record [--out base]]             # live leader→follower, optional .json+.rrd recording
dume run record-dataset ...                                     # wrapper over lerobot-record for VLA demos
dume run record-batch [--batches N] [--push]                   # structured session: batches of 3 episodes (negro/verde/rojo)
dume run eval --color red|green|black [--n N] [--record]        # run trained policy on follower (no leader needed)
dume run eval-viz --color red|green|black [--fps 15]            # same as eval + rerun ResNet-18 activation heatmap
dume run delete-batch N [--dry-run]                             # delete 3 episodes of batch N (1-indexed) from local dataset
dume run push dataset|model [...]                               # upload dataset or trained checkpoint to HF Hub
dume run pull dataset|model [--repo-id ID]                      # download dataset (→ lerobot cache) or model (→ hub cache)
dume run train [--type smolvla|act] [--device cuda]             # wrap lerobot-train; default → <HF_USER>/smolvla_terminal_sort
dume run dataset-stats [--repo-id ID]                           # episodes/frames balance per color (pre-train sanity check)
```

Full flag reference for all `dume run` subcommands: `so101_cli/COMMANDS.md`.

```bash
# DUM-E: TUI rica en iconos (Textual). `dume` abre la TUI; `dume run <subcmd>` es el CLI scriptable.
./dume                 # cockpit (home): conexión, brazos, datasets, modelos, quick actions
./dume <view>          # salta a una vista TUI (home + teleop son reales; record/eval/train... placeholder hasta fases sig.)
./dume --ascii         # sin Nerd Font ni imágenes inline
./dume run <subcmd>    # modo CLI (el antiguo `cal`): teleop, record-batch, eval, train, ... (ver lista arriba)
```
...
y corre `dume run` para ops pesadas.
...
2. **`so101_cli/` Python package** — project-owned code for custom motion behavior LeRobot doesn't ship: smooth interpolated moves to named poses, leader-side keyboard-driven recording, replay with smoothstep blending, and rerun-based visualization. It imports LeRobot's robot/teleop classes directly (`SOFollower`, `SOLeader`), bypassing the LeRobot CLI. Exposed two ways, both over the same modules: the **`dume` TUI** and the scriptable **`dume run <subcommand>`** CLI (the former standalone `cal` command was removed; `dume run` is its 1:1 replacement).

`cli.py` builds the `dume run` argparse tree; each subcommand lives in its own module and registers via `add_*_parser(sub)`. The dispatch convention is `args.func(args)` — every parser sets `set_defaults(func=...)`. `so101_cli/dume/__main__.py` routes a leading `run` token here; otherwise it launches the TUI.
...
The TUI front-end (Textual) — the second face of the `so101_cli` package alongside the `dume run` CLI — see `docs/dume.md` for phases/packaging. It **never reimplements robot logic**: it reuses the `so101_cli` modules by import for read-only/display work, and for anything heavy or interactive it suspends the TUI and shells out to `dume run <subcmd>`. Internal structure:
...
- **`engine/`** — the non-UI core, all reusing `so101_cli` modules without touching them:
  - `runner.py::run_cli(app, subcmd, *args)` — the **only** way to run heavy ops: `with app.suspend()` → `subprocess.call([sys.executable, "-m", "so101_cli.dume", "run", subcmd, ...])` → wait for Enter → return code. New record/eval/train/push actions go through this, never reimplement `dume run` flows.
...
- **View pattern (Phase 2+, see `screens/teleop.py`)** — editable form fields → a `CommandPreview` widget that recomposes `$ dume run <subcmd> …` live → a **Launch** button that calls `run_cli`. Views never re-implement behavior; they compose flags and delegate 1:1 to a `dume run` subcommand.
...
- **Output paths default to `paths/teleop_<timestamp>`** for `dume run teleop --record`. `paths/`, `poses/` (recorded `trajectory` JSONs like `wave.json`, `salute.json`), `outputs/`, `zed_captures/`, the root `photo*.png`, and `x.py` are scratch/capture artifacts — not part of the architecture; don't wire them into commands.
